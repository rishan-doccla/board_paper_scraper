...
latest_results = {"last_run": None, "board_papers": []}
    ...
    if os.path.exists(RESULTS_FILE):
        with open(RESULTS_FILE, "r") as f:
            ...
            if "board_papers" in data:
                data["board_papers"] = [
                    paper
                    for paper in data["board_papers"]
    ...
    try:
        ...
        for term, data in results.get("terms_data", {}).items():
            ...
            if quotes and summaries:
                clean_terms_data[term] = {"quotes": quotes, "summaries": summaries}
...
def create_paper_dict(paper, org_url, existing_papers):
    ...
    filename = (
        ...
        if paper.get("title") != "Unknown"
        else paper["url"].split("/")[-1]
    )
    ...
    for paper in papers:
        ...
        if not scrape_only:
            ...
            if pdf_analyzer.is_from_2024_or_later(date):
                ...
                analysis_results = process_paper_analysis(
                    paper_dict, pdf_analyzer, scrape_only
                )
                paper_dict.update(
                    {
        ...
        elif paper_dict.get("date") != "Unknown" and pdf_analyzer.is_from_2024_or_later(
            paper_dict.get("date")
        ):
...
def run_crawler():
    ...
    existing_papers = {
        ...
        for paper in existing_data.get("board_papers", [])
    }
...
def trigger_crawler():
    ...
    run_crawler()
    return jsonify(
        {"status": "success", "message": "Crawler completed", "results": latest_results}
    )
...
def test_specific_urls():
    ...
    urls = data.get("urls", [])
    scrape_only = data.get(
        "scrape_only", False
    )  # Get scrape_only parameter from request
    ...
    results = {"last_run": datetime.datetime.now().isoformat(), "board_papers": papers}
...
def analyze_papers():
    ...
    try:
        ...
        current_papers = {
            paper["url"]: paper for paper in latest_results.get("board_papers", [])
        }
        results = []
        ...
        for url in urls:
            if url in current_papers and current_papers[url].get("terms_count", 0) > 0:
                ...
                try:
                    ...
                    print(
                        f"Paper from 2024 or later ({date}), performing full analysis"
                    )
                    ...
                    results.append(
                        {
                            "url": url,
                            "title": current_papers.get(url, {}).get(
                                "title", "Unknown"
                            ),
                            "has_relevant_terms": False,
                            "terms_found": [],
        ...
        if paper["url"] == url:
            ...
            for term, data in analysis.get("terms_data", {}).items():
                ...
                if isinstance(quotes, str):
                    ...
                    # Apply cleaning to each quote
                    quotes = [
                        pdf_analyzer.clean_repetitive_text(q) for q in quotes if q
                    ]
                ...
                if isinstance(summaries, str):
                    ...
                    # Apply cleaning to each summary
                    summaries = [
                        pdf_analyzer.clean_repetitive_text(s) for s in summaries if s
                    ]
            ...
            if analysis.get("date") and analysis["date"] != "Unknown":
                ...
                paper["sort_date"] = (
                    analysis["date"]
                    if pdf_analyzer.is_from_2024_or_later(analysis["date"])
                    else "9999-99"
                )
...
def reset_existing_papers():
    ...
    """
    empty_results = {"last_run": None, "board_papers": []}
...
def debug_paper(index):
    ...
    if index >= len(latest_results["board_papers"]):
        return (
            jsonify(
                {
                    "error": f"Paper index {index} out of range. Only {len(latest_results['board_papers'])} papers available."
                }
            ),
            404,
        )
...
def export_detailed_results():
    ...
    # Define CSV Headers
    headers = ["Organisation", "Date", "Title", "Term", "Summary", "Mentions", "Link"]
    ...
    response.headers["Content-Disposition"] = (
        "attachment; filename=board_papers_detailed_export.csv"
    )
    return response
